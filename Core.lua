...
function Guildbook:Init()
    ...
    -- grab version number
    self.version = tonumber(GetAddOnMetadata(addonName, "Version"))
...
function Guildbook:GetPaperDollStats()
    if GUILDBOOK_CHARACTER then
        ...
        GUILDBOOK_CHARACTER['PaperDollStats'].SpellHit = 0 -- self:TrimNumber(GetCombatRatingBonus(CR_HIT_SPELL) + GetSpellHitModifier());
        GUILDBOOK_CHARACTER['PaperDollStats'].MeleeHit = 0 --self:TrimNumber(GetCombatRatingBonus(CR_HIT_MELEE) + GetHitModifier());
	    GUILDBOOK_CHARACTER['PaperDollStats'].RangedHit = 0 -- self:TrimNumber(GetCombatRatingBonus(CR_HIT_RANGED));
